...
router.post("/affiliate/click", optionalFirebaseAuth, async (req, res) => {
  try {
    const { merchant_id, offer_id, ip_address, user_agent, anon_id } = req.body;
    const userId = req.user?.id ?? null;
    console.log("Affiliate Click Route hit", { merchant_id, offer_id, ip_address, user_agent, anon_id, userId });
   ...
   const result = await zingoPool.query(
      `WITH merchant AS (
            ...
            INSERT INTO affiliate_clicks
              (click_id, user_id, anon_id, merchant_id, offer_id, destination_url, ip_address, user_agent)
            SELECT $2::uuid, $3, $7::uuid, $1, $4,
                  replace(replace(target.raw_url, '{click_id}', $2::text), '{anon_id}', COALESCE($7::text, '')),
                  $5, $6
            FROM target
      ...
      [
        ...
        ip_address,
        user_agent,
        anon_id || null, // Ensures empty strings "" become null so $7::uuid doesn't fail
      ]
    );

    if (result.rows.length === 0) {
      return res.status(422).json({
        error: "Merchant has no tracking URL configured, or offer/merchant not found",
      });
    }
